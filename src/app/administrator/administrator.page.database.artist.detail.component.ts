...
export class AdministratorPageDatabaseArtistDetailComponent implements OnInit, OnDestroy {
  ...
  addVideoUrl(url: string) {
    this.database.list('database/artist/' + this.id + '/videoList').push(url).then(result => {
      (document.getElementById('form-iframe-close-btn') as HTMLElement).click();
    });
  }
  editVideoUrl(key: string, url: string) {
    this.database.object('database/artist/' + this.id + '/videoList/' + key)
      .set(url)
      .then(result => {
        (document.getElementById('form-iframe-close-btn') as HTMLElement).click();
      });

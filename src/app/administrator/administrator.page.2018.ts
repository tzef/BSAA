...
@Component({
  template: `
    <div class="container">
      <div class="row">
        <div class="col">
          <app-page-title-component title="2018 報名資料" topImage=false></app-page-title-component>
        </div>
      </div>
    </div>
...
export class AdministratorPage2018Component implements OnInit, OnDestroy {
  ...
  getInfo() {
    ...
    this.historySubscription = this.database
      .list('plan/form/2018', ref => ref.orderByChild('date'))
      .snapshotChanges()
      .pipe(map(action => {
